...
# Tell Python to also look in the parent folder (main) for imports
...
mcp = FastMCP("GreaterNoidaRealEstate")
...
class AzureMapsTools:
    ...
    def _get_coordinates(self, society_name: str) -> Optional[tuple]:
        q = society_name if "Greater Noida" in society_name else f"{society_name}, Greater Noida"
    ...
    def find_nearby_amenity(self, society_name: str, amenity_type: str, radius: int = 5000, limit: int = 10) -> str:
        try:
            ...
            filtered = [
                ...
                for item in results if "Sector" not in item.get("poi", {}).get("name", "")
            ]
...
class MarketAnalysisTools:
    ...
    def get_market_context(self, locality: str):
        ...
        df = pd.read_sql_query(f"SELECT rent, area_sqft FROM nobroker_cleaned WHERE Locality = '{locality}'", conn)
        conn.close()
        if df.empty:
            return None
    ...
    def evaluate_deal(self, rent: int, area_sqft: int, locality: str) -> str:
        ...
        def to_k(amount): return f"₹{amount/1000:.1f}k".replace(".0k", "k")
...
# ─── Expose the 2 Tools to MCP ──────────────────────────────────────────────
azure_maps = AzureMapsTools()
market_analysis = MarketAnalysisTools(db_path=DB_PATH)

@mcp.tool()
def find_nearby_amenity(society_name: str, amenity_type: str, radius: int = 5000, limit: int = 10) -> str:
    """Find nearby amenities (schools, hospitals, malls, etc.) near a given society."""
    return azure_maps.find_nearby_amenity(society_name, amenity_type, radius, limit)

@mcp.tool()
def evaluate_deal(rent: int, area_sqft: int, locality: str) -> str:
    """Evaluate whether a property deal is fair, overpriced, or a bargain based on local median rates."""
    return market_analysis.evaluate_deal(rent, area_sqft, locality)

if __name__ == "__main__":
    # Host the tools on a local SSE web server
